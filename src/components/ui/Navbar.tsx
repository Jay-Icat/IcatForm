...
import { Volume2, VolumeX, Shield, Users2 } from "lucide-react";
import { useQuiz } from "@/context/QuizContext";

export function Navbar() {
  const { isMuted, toggleMute, team } = useQuiz();
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-40 px-3 sm:px-8 py-3 sm:py-4 flex items-center justify-between pointer-events-auto">
      {/* Whole ICAT Brand Logo & Team Name */}
      <div className="flex items-center gap-3">
        ...
        {team && team.id !== "default" && (
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/30 backdrop-blur-md">
            <Users2 className="w-3.5 h-3.5 text-red-400" />
